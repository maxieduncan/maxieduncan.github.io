...
Infrastructure as Code means that I can now role out a new environment into an existing AWS account in minutes and deploy our entire stack into a new AWS account in an hour or so, compared to the days it would take to manually create and configure the resources. This allows us to create developer environments on the fly, quickly deploy new environments for testing feature branches and to isolate our production and test environments by running a single command. It's a great improvement to manually creating and configuring the AWS resources using the console or command line; it's a huge improvement to the days of having to first provision additional hardware then go through the rigmarole of installing different software platforms, configuring and then deploying.
